        ...
        <div class="rightBox">
            <div class="user-attendanceR-list">
                <div class="user-attendanceR-detail">
                    ...
                    <span class="time">考勤时间</span>
                    <span class="time">出勤人数</span>
                    <span class="time">缺勤人数</span>
                    <span class="time">请假人数</span>
                </div>
            </div>
            <?php
                include 'conn.php';
                $sql = "call show_allsignIn_$user('$id')";
            ...
            <div class="user-attendanceR-list ">
                <div class="user-attendanceR-detail">
                    ...
                    <?php
                        include 'conn.php';
                        $sql = "call countStudent('".$row['cid']."','".$row['sign_time']."')";
                        $count_result=mysqli_query($mysql, $sql);
                        $count_num = @mysqli_field_count($mysql);
                        $count_row = @mysqli_num_rows($count_result);
                        mysqli_close($mysql);
                        $row=@mysqli_fetch_array($count_result);
                        if ($row[0]=="出勤") {
                            echo '<span class="time">'.$row[1].'</span>';
                            $row=@mysqli_fetch_array($count_result);
                            if ($row[0]=="缺勤") {
                                echo '<span class="time">'.$row[1].'</span>';
                                $row=@mysqli_fetch_array($count_result);
